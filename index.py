...
def handler(event, context):
    users = json.loads(os.getenv('app-user'))
    for u in users:
        ...
        if st < n < et or '-force' in sys.argv:
            if us['data']['lasterrow'] is None or '-force' in sys.argv:
                ...
                t = uci['data']['sign_at']
                m = uci['msg']
                return t, 'xgh: ' + u['xgh'], 'id: ' + i, m
            else:
                t = datetime.datetime.strptime(us['data']['lasterrow']['sign_at'], '%Y-%m-%d %H:%M:%S').strftime(
                    '%H:%M:%S')
                n = n.strftime('%Y-%m-%d %H:%M:%S')
                return n, 'xgh: ' + u['xgh'], 'id: ' + i, f'{t}に手動でチェックインしました。「-force」パラメータで強制チェックインをします（リスクある）'
        else:
            n = n.strftime('%Y-%m-%d %H:%M:%S')
            return n, 'xgh: ' + u['xgh'], 'id: ' + i, 'チェックインすべき時間の範囲外です。「-force」パラメータで強制チェックインをします（リスクある）'
